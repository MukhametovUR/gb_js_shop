...
const app = new Vue({
    ...
    methods: {
        getJson(url){
            return fetch(url)//При помощи fetch коннектимся к url - /api/products
           //В файле server.js есть функция обработки url - app.get  
            //Передаем строку json(products.json) при поможи переменной
            //data (res.send(data)) из метода app.get файла server.js   
            .then(result => result.json())
            //Получив ответ, преобразуем строку из data (res.send(data)) 
            //в строку объектов JS
                .catch(error => {
                    // console.log(error)
